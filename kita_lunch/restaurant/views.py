...
class TopPageView(generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if "select_session" in self.request.session:
            self.request.session["select_session"] = "-created_at"
        ...
        context["restaurant_list"] = new_restaurant_list
...
class RestaurantListView(generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        select_sort = select_sort if select_sort is not None else ""
        ...
        restaurant_list = models.Restaurant.objects.filter(
            Q(name__icontains=keyword_session) | Q(description__icontains=keyword_session)
        )
...
class ReservationListView(generic.ListView):
    ...
    def get_queryset(self):
        user_id = self.request.user.id
        queryset = models.Reservation.objects.filter(user_id=user_id).order_by("-date")
        return queryset
...
class ReviewListView(generic.ListView):
    ...
    def get_queryset(self):
        ...
        user_id = self.request.user.id
        queryset = models.Review.objects.filter(user_id=user_id).order_by("-rate")
